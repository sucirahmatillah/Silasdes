...
namespace App\Http\Requests\User\Surat;
...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        if ($this->jenis_surat->id == 'surat-keterangan-domisili') {
            $rules = [
                ...
                'tanggal_surat' => ['required', 'date_format:Y-m-d', Rule::unique('surat', 'tanggal_surat')->where('id_penduduk', $this->input('id_penduduk'))->where('id_jenis_surat', $this->jenis_surat->id)->ignore($this->surat->id ?? null, 'id')],
    ...
    {
        return [
            'tanggal_surat.unique' => 'Penduduk (Pemohon) pada tanggal ini sudah mengajukan ' . $this->jenis_surat->nama,
        ];
    }
