...
@section('content')
    <form id="form_submit" class="card card-md" action="{{ route('penduduk.password.email') }}" method="POST">
        @csrf
        <div class="card-body">
            ...
            <div class="form-footer d-grid gap-2">
                ...
                <a href="{{ route('penduduk.login') }}" class="btn btn-outline-primary">Kembali</a>
            </div>
        </div>
    </form>
@endsection
...
@push('script')
    {!! JsValidator::formRequest('App\Http\Requests\Penduduk\Auth\ForgotPasswordRequest', '#form_submit') !!}
@endpush
